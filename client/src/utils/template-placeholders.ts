...
export const SAMPLE_VALUES: Record<string, string> = {
  ...
  // 10.1 — контракт рекомендаций критики (словарь данных предпросмотра; панель
  // рекомендаций — беседа 10.3). Требует сторож 4ah: плейсхолдеры серверных
  // шаблонов обязаны иметь образец, иначе предпросмотр админки показывает дыры.
  document_subsections:
    "  Граф категорий: Методология построения графа | Таблица категорий | Таблица связей | Топология графа | Топологическая таблица\n  Корпус тезисов: Онтологические тезисы | Эпистемологические тезисы | Сводная таблица тезисов",
  recommendations_prose: "[текст подраздела «Рекомендации по улучшению»]",
  critique_subsections: "— Внутренняя когерентность\n— Верность методу синтеза\n— Слепые пятна",
  categories: "— Бытие\n— Dasein",
  theses: "— О-1 — Бытие предшествует сущему\n— Э-1 — Понимание исторично",
  terms: "— Бытие\n— Забота",
  table_contract: "[контракт подраздела «Таблица рекомендаций»]",
  ...
  note: "Операция: уточнить формулировку. Основание: «Внутренняя когерентность». [текст рекомендации]",
