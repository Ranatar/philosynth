...
// части 3 globals.css.
{
  ...
  need(await cm("../client/src/components/edit/CategoryEditor.tsx"),
    ["CategoryEditor", "categoryToDraft", "categoryDiff", "CATEGORY_TYPES"], "client/edit/CategoryEditor");
  ...
  // (в) CATEGORY_TYPES ⊆ канонического списка промпта графа (временная
  //     константа до TaxonomySelector 5.4 — §12)
  const st4ac = await import("./config/section-templates.js") as Record<string, unknown>;
  const ce4ac = await cm("../client/src/components/edit/CategoryEditor.tsx") as { CATEGORY_TYPES: readonly string[] };
  const stText = JSON.stringify(st4ac);
  for (const t of ce4ac.CATEGORY_TYPES)
    if (!stText.includes(t)) errs.push(`4ac: тип «${t}» из CATEGORY_TYPES не встречается в section-templates`);
