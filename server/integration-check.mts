...
// правила блока 9 кита дословно в части 3 globals.css.
{
  ...
  need(tapi, ["transformGraphToTheses", "transformThesesToGraph", "startTransformRequest", "getTransformHistory", "rollbackTransform"], "client/api/transforms (5.5)");
  ...
  try {
    await (tapi.transformGraphToTheses as (...a: unknown[]) => Promise<unknown>)("S");
    await (tapi.transformThesesToGraph as (...a: unknown[]) => Promise<unknown>)("S");
...
// перенесены дословно.
{
  ...
  need(apiP, ["listPrompts", "getVersions", "getTemplateVersions", "createVersion", "activateVersion", "listConfigs", "updateConfig", "getConfigVersions", "getConfigVersionsFull", "activateConfigVersion"], "client/api/prompts");
  ...
  try {
    ...
    await Promise.all([
      ...
      (apiP.listPrompts as () => Promise<unknown>)().catch(() => 0), (apiP.getVersions as (k: string) => Promise<unknown>)(key), (apiP.createVersion as (k: string, b: string) => Promise<unknown>)(key, "b"), (apiP.activateVersion as (k: string, v: number) => Promise<unknown>)(key, 1),
      (apiP.listConfigs as () => Promise<unknown>)().catch(() => 0), (apiP.updateConfig as (k: string, v: unknown) => Promise<unknown>)(key, {}), (apiP.getConfigVersions as (k: string) => Promise<unknown>)(key), (apiP.activateConfigVersion as (k: string, v: number) => Promise<unknown>)(key, 1),
    ]);
