...
// «Доступ» (статус после списка И журнала, своя строка без кнопки, roleClass).
{
  ...
  for (const s of ['outcome: "skip"', 'outcome: "updated"', 'outcome: "created"', 'outcome: "fail"', "pg_advisory_xact_lock", "USER_BOOTSTRAPPED", 'e.cause?.code === "23505"', 'from "../server/middleware/auth.js"']) if (!bootS.includes(s)) errs.push(`4aj: bootstrap-admin без «${s}»`);
  const pkg = JSON.parse(await rd("../package.json")) as { scripts: Record<string, string> };
...
// сеет идемпотентно, termux-скрипт), тесты не сносят посеянный starter.
{
  ...
  const termux = await rd("../scripts/philosynth-termux.sh");
...
async function section4ao(): Promise<void> {
  ...
  if (journal.entries.at(-1)?.tag !== "0005_visibility") errs.push("4ao: журнал миграций без 0005_visibility последней");
...
async function section4ap(): Promise<void> {
  ...
  // тесты и метки
  for (const t of ["../tests/smoke-87-request1.mjs", "../tests/test-87-requests2-12.mjs", "../scripts/patch-docs-conv87.py"]) { try { await fsm.access(new URL(t, import.meta.url)); } catch { errs.push(`4ap: нет ${t}`); } }
