...
async function section5ac(): Promise<void> {
  ...
  try {
    ...
    // (д) toPreview/projectPreview: isPublic производный, гость без стоимости
    const pv = rs.toPreview(rows.find((r) => r.id === show)!, ["Кант"], false, an.get(show));
    if (pv.isPublic !== true || pv.visibility !== "showcase" || pv.authorName !== `Автор ${tag}` || typeof pv.totalCostUsd !== "number") errs.push("5ac: toPreview — isPublic/visibility/authorName/стоимость");
    ...
    if (rs.toPreview(rows.find((r) => r.id === priv)!, []).isPublic !== false) errs.push("5ac: приватная isPublic не false");
  } finally {
...
async function section4ao(): Promise<void> {
  ...
  for (const f of ["syntheses", "lineage", "logs", "export", "sections", "elements", "modes", "transforms", "enrichment", "plans", "billing", "import", "generation"]) {
    const c = strip(await rd(`./routes/${f}.ts`)).replace(/isPublic: isPublicOf\(row\.visibility\)/g, "").replace(/body\.isPublic|details\.isPublic/g, "");
  ...
  // (д) PATCH: visibility + флаги, isPublic — синоним; duplicate → private
  ...
  if (!/patch\.visibility = body\.isPublic \? "full" : "private"/.test(synC) || !/устаревший синоним visibility — не вместе с ней/.test(synS)) errs.push("4ao: PATCH — синоним isPublic не по правилу (true→full, false→private, вместе с visibility → 400)");
