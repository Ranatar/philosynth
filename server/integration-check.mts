...
// (pg_constraint); updateCustomType с пустым patch строки не пишет.
{
  ...
  const bootPath = "../scripts/bootstrap-admin.ts";
...
// на них, а не на тестовом плане.
{
  ...
  const seedMod = (await import("../scripts/seed-plans.ts" as string)) as { seedPlans: (o: Record<string, unknown>) => Promise<{ results: { name: string; outcome: string; error?: string }[]; counts: Record<string, number> }> };
...
// «Доступ» (статус после списка И журнала, своя строка без кнопки, roleClass).
{
  ...
  // bootstrap-admin
  const bootS = await rd("../scripts/bootstrap-admin.ts");
  ...
  const pkg = JSON.parse(await rd("../package.json")) as { scripts: Record<string, string> };
  if (pkg.scripts["seed:admin"] !== "tsx scripts/bootstrap-admin.ts") errs.push("4aj: npm-скрипт seed:admin отсутствует");
...
// сеет идемпотентно, termux-скрипт), тесты не сносят посеянный starter.
{
  ...
  need(await import("../scripts/seed-plans.ts" as string), ["seedPlans", "printReport"], "scripts/seed-plans");
  need(await import("../scripts/stripe-create-prices.ts" as string), ["createPrices", "envLines", "priceMatchesPlan", "priceCents", "STRIPE_PRODUCT_META_KEY"], "scripts/stripe-create-prices");
  ...
  // 4al — seed-plans
  const seedS = await rd("../scripts/seed-plans.ts");
  ...
  const scpS = await rd("../scripts/stripe-create-prices.ts");
  ...
  // 4al — обвязка
  const pkg = JSON.parse(await rd("../package.json")) as { scripts: Record<string, string> };
  if (pkg.scripts["seed:plans"] !== "tsx scripts/seed-plans.ts" || pkg.scripts["stripe:create-prices"] !== "tsx scripts/stripe-create-prices.ts") errs.push("4al: npm-скрипты seed:plans / stripe:create-prices");
  const envEx = await rd("../.env.example");
