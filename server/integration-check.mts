...
async function section4as(): Promise<void> {
  ...
  if (JSON.stringify(paths) !== JSON.stringify(["get /:id/recommendations", "post /:id/recommendations/parse", "post /:id/recommendations/extract"])) errs.push(`4as: маршруты рекомендаций: ${paths.join(", ")}`);
  if ((routes.match(/ownerEditGate\(c, id, user\.id\)/g) ?? []).length !== 2 || !/ownerReadGate\(c, id, user\.id\)/.test(routes) || /loadSynthesisForRead/.test(routes)) errs.push("4as: рекомендации доступны только владельцу — GET через ownerReadGate, оба POST через ownerEditGate, без loadSynthesisForRead");
...
// у каждого withGenerationSlot задана квота.
{
  ...
  const slotQuotas = (svcAll.match(/\}, \{ quota: .*?\}\); \/\/ 6\.1: квота подписки/g) ?? []).length + (svcAll.match(/^\s*\{ quota: (?:"syntheses"|null)(?:, estimatedCostUsd: opts\.estimatedCostUsd)? \},\n\s*\);/gm) ?? []).length;
