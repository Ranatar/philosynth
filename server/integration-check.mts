...
// у каждого withGenerationSlot задана квота.
{
  ...
  const slotQuotas = (svcAll.match(/\}, \{ quota: .*?\}\); \/\/ 6\.1: квота подписки/g) ?? []).length + (svcAll.match(/^\s*\{ quota: (?:"syntheses"|null) \},\n\s*\);/gm) ?? []).length;
