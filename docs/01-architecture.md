...
- Парсинг: `parseGraph()` (parseGraph()) + нормализация типов через Element Taxonomy (4.8) и `parseTopology()` (parseTopology()) адаптируются для извлечения из HTML-ответа Claude и записи в БД. ФАКТ 5.5 (2026-09-06): нормализация на каталог — `normalizeGraphTypesToCatalog` в graph-parser, вызывается из `saveGraphToDb` после транзакции (fail-open; текст типа не меняется; до 5.5 `type_catalog_id` заполнялся лишь через PATCH/TaxonomySelector)
...
## 8. Оформление клиента: единство стилей с исходником
