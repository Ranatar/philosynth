...
> **Ревизия 2026-07-22**: задачи фаз дополнены подсистемами v11 —
> pause/resume (MVP: G8/G9), селективный родительский контекст + режим
> бюджета, кардинальность/свободный синтез, MODE_DEPS. Спецификации:
> Сессия 1 уже выполнена по старой редакции — её артефакты
> требуют миграции 0001 (поля v11 из 02) и обновления констант.
...
2. Drizzle ORM: схема БД (26+ таблиц по 02-data-model, включая поля v11:
   keep_full_budget, parent_context_schema, paused_state; budget_mode/
   parent_spec в context_log; маркеры паузы в generation_log).
   Для уже выполненной сессии 1 — миграция 0001 с этими полями
...
8. `scripts/seed-prompts.ts`: извлечение всех промптов из исходника → `prompt_templates`
   > v10: список философов расширен на 36 позиций — всего 106; в репо
   > (сессия 1) пока 74 — обновить `shared/constants/philosophers.ts`
9. `scripts/seed-configs.ts`: извлечение конфигов → `synthesis_configs`.
   v11: + parent_deps.base/genetic/level/level_genetic/method,
   parent_intra_deps, parent_field_order/labels, md_by_card, sd_by_card,
   mode_deps; context_deps.level/_genetic/method вместо *_deps_patch
...
>
> **v11**: (а) все промптовые ветки зависят от `participantCardinality()`
> (none/single/multi; MD_BY_CARD/SD_BY_CARD) — свободный синтез с 0
> участников обязан работать с первой генерации; (б) чекбокс
> `keepFullBudget` (мета-синтез); (в) стриминг строится по модели
> `_streamRespOnce` + классификация ошибок; пауза/резюм — MVP (G8/G9).
...
- `compat-advisor.ts`: перенос computeSectionRating + Advisor v2
  (getCompatEntryByKey, severity, чипы; computeOverallCompat/
  computeMethodRating/getCompatAdvice в исходнике удалены)
...
- `context-builder.ts`: адаптация buildContextForSection (DOM → БД);
  v11: `parentOverheadForSection()` + `applyBudgetPressure()` (пол 40%),
  запись budget_mode/parent_overhead/parent_spec в context_log
- `context-quality.ts`: getSectionContextQuality поверх context_log (v11)
...
**1.3. Стриминг + пауза (бэкенд + клиент)**
- `streaming-manager.ts`: Claude SSE → парсинг → буферизация.
  Модель v11: `_streamRespOnce` (одна попытка + классификация kind:
  auth/billing/pre-stream/max-tokens/partial/stuck/user-abort);
  ретраи только pre-stream (1с/3с/8с), stuck-таймаут 45с
- `pause-resume-service.ts`: pausedState (gen|plan) в syntheses.paused_state,
  серверные оценки (_computeGenPauseEstimates), ветки
  fill-missing-subs/retry/skip/stop (01 §4.12) — MVP, G8/G9
- `PauseModal.tsx`: 4 рендерера (gen/plan/billing/auth)
...
- `CostEstimate.tsx`, `CompatAdvisor.tsx` (entry-модель v2)
- `SectionWarnings.tsx` (v11: предупреждения/рекомендации/подстановки)
- `GenerationProgress.tsx`
- Валидация формы v11: 0 участников допустимо (свободный синтез,
  обязателен seed); чекбокс keepFullBudget при наличии концепций в пуле
...
> v11: formatCtxLog/colorizeLog включают правила tz_budget_mode 4.8.3
> (Контекст родителей, режим бюджета, per-parent breakdown,
> ⚠ отсутствует обязательное поле, ↻ МИГРАЦИЯ СХЕМЫ) и маркеры
> pause/resume/user-action (в «Логе контекста», но не в «Логе промптов»).
...
**3.1. Мета-синтез**
- `meta-synthesis-service.ts`: загрузка контекста участника из БД;
  `conceptContextBlockSelective` — состав полей пер-секционный по
  PARENT_DEPS (`parent-context.ts`: resolveParentDeps(ForSubsection));
  миграция схемы monolithic → selective-v1 с маркером в genLog;
  keepFullBudget: пересинхронизация перед каждой операцией (5.3)
...
### Фаза 9: Самостоятельность пользователя (≈2 сессии)
...
| **ИТОГО** | **34–45** (+2–4 на v11: pause-resume, parent-context, кардинальные ветки промптов, миграция 0001) | |
