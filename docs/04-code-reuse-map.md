...
| CSS исходника [13–3528] ЦЕЛИКОМ (не только документная часть: `.doc-*`, `#docTOC`, `.callout*`, `.risk*`, но и `.topbar`, `.site-header`, `.input-form`, `.form-*`, `.checkboxes-row`, `.sec-*`, `.compat-*`, `.pool-*`, `.progress-*`, `.pause-*`, `.cascade-*`, `.edit-*`, `.mode-*`, `.gm-*`, `.gen-*`) | `client/src/globals.css` — ДОСЛОВНЫЙ порт, правило в правило, в исходном порядке (часть 2 файла; правка 2026-09-02). Документные классы обязательны для рендера html_content через dangerouslySetInnerHTML; остальные — оформление интерфейса, компоненты используют их вместо утилит Tailwind |
