...
function pickHtml(prompt) {
  ...
  if (task.includes("Таблица рекомендаций")) return critiqueHtml42();
...
try {
  ...
  /* ════ R5: тест 5 07 — roundtrip-импорт (покрыт беседой 4.3) ════ */
  console.log("\n■ R5 (тест 5): roundtrip-импорт — покрыт тестом беседы 4.3");
  {
    /* Правка 2026-09-02 (ревизия устаревших ожиданий): до 4.3 роут
       отсутствовал и проверка ждала 404. Роут существует с беседы 4.3;
       пустое тело без файла — это 400 (multipart-валидация), а не 404. */
    const r = await api(A, "POST", "/syntheses/import", {});
    ok(r.status === 400, "POST /syntheses/import есть (беседа 4.3), пустое тело → 400",
      `status=${r.status}`);
    console.log("  → сам roundtrip покрыт первым тестовым запросом беседы 4.3");
  }
